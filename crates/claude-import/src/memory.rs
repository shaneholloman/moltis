//! Import memory files from Claude Code.
//!
//! Copies `CLAUDE.md` from `~/.claude/` as a workspace reference file.
...
pub fn import_memory(detection: &ClaudeDetection, dest_dir: &Path) -> CategoryReport {
    ...
    if dest.is_file() {
        debug!("CLAUDE.md already exists in destination, skipping");
    ...
    match std::fs::copy(source, &dest) {
        ...
        Err(e) => CategoryReport::failed(
            ImportCategory::Memory,
            format!("failed to copy CLAUDE.md: {e}"),
        ),
    }
}
...
mod tests {
    ...
    fn import_memory_skips_existing() {
        ...
        std::fs::write(dest_dir.join("CLAUDE.md"), "existing content").unwrap();
