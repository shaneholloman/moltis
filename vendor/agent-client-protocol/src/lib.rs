use {
    futures::{AsyncRead, AsyncWrite, future::LocalBoxFuture},
    rpc::RpcConnection,
};
...
impl ClientSideConnection {
    ...
    /// Creates a client-side connection with explicit transport resource limits.
    pub fn new_with_limits(
        ...
        let (conn, io_task) =
            RpcConnection::new_with_limits(client, outgoing_bytes, incoming_bytes, limits, spawn);
        (Self { conn }, io_task)
    }
...
impl AgentSideConnection {
    ...
    /// Creates an agent-side connection with explicit transport resource limits.
    pub fn new_with_limits(
        agent: impl MessageHandler<AgentSide> + 'static,
        outgoing_bytes: impl Unpin + AsyncWrite,
        incoming_bytes: impl Unpin + AsyncRead,
        ...
        let (conn, io_task) =
            RpcConnection::new_with_limits(agent, outgoing_bytes, incoming_bytes, limits, spawn);
        (Self { conn }, io_task)
    }
