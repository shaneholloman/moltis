...
use futures::{AsyncWrite, AsyncWriteExt as _, channel::oneshot};
...
impl TestAgent {
    ...
    fn with_blocked_prompts(mut self) -> Self {
        self.block_prompts = true;
        self
    }
}
...
#[tokio::test]
async fn blocked_stdout_does_not_block_cancellation_or_eof() {
    ...
    local_set
        .run_until(async {
            ...
            incoming_tx
                .write_all(b"{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"unknown\",\"params\":{}}\n")
                .await
                .unwrap();
            tokio::time::timeout(Duration::from_secs(1), write_started_rx)
                .await
                .unwrap()
                .unwrap();

            incoming_tx
                .write_all(b"{\"jsonrpc\":\"2.0\",\"method\":\"session/cancel\",\"params\":{\"sessionId\":\"test\"}}\n")
                .await
                .unwrap();
            tokio::time::timeout(Duration::from_secs(1), async {
                while agent.cancellations_received.lock().unwrap().is_empty() {
            ...
            incoming_tx.close().await.unwrap();
            let io_result = tokio::time::timeout(Duration::from_secs(1), io_handle)
                .await
                .unwrap()
                .unwrap();
...
#[tokio::test]
async fn write_failure_fails_connection_and_pending_request() {
    let local_set = tokio::task::LocalSet::new();
    local_set
        .run_until(async {
            let client = TestClient::new();
            let (incoming_rx, _incoming_tx) = piper::pipe(1024);
            let (connection, io_task) =
                ClientSideConnection::new(client, FailingWriter, incoming_rx, |future| {
                    tokio::task::spawn_local(future);
                });
            let io_handle = tokio::task::spawn_local(io_task);
            let request = connection.initialize(
                InitializeRequest::new(ProtocolVersion::LATEST)
                    .client_info(Implementation::new("test-client", "0.0.0")),
            );

            let (request_result, io_result) = tokio::time::timeout(
                Duration::from_secs(1),
                futures::future::join(request, io_handle),
            )
            .await
            .unwrap();
            assert!(request_result.is_err());
            assert!(io_result.unwrap().is_err());
        })
        .await;
}

#[tokio::test]
async fn dropping_unpolled_io_task_fails_pending_request() {
    let local_set = tokio::task::LocalSet::new();
    local_set
        .run_until(async {
            let client = TestClient::new();
            let (incoming_rx, _incoming_tx) = piper::pipe(1024);
            let (connection, io_task) =
                ClientSideConnection::new(client, futures::io::sink(), incoming_rx, |future| {
                    tokio::task::spawn_local(future);
                });
            let request = connection.initialize(
                InitializeRequest::new(ProtocolVersion::LATEST)
...
#[tokio::test]
async fn oversized_caller_message_does_not_close_connection() {
    let local_set = tokio::task::LocalSet::new();
    local_set
        .run_until(async {
            let client = TestClient::new();
            let (incoming_rx, _incoming_tx) = piper::pipe(1024);
            let (connection, _io_task) = ClientSideConnection::new_with_limits(
                client,
                futures::io::sink(),
                incoming_rx,
                ConnectionLimits::bounded(256, 256, 16),
                |future| {
                    tokio::task::spawn_local(future);
                },
            );
            ...
            let oversized = connection
                .cancel(CancelNotification::new(SessionId::new("x".repeat(256))))
                .await;
            assert!(oversized.is_err());
            connection
                .cancel(CancelNotification::new(SessionId::new("small")))
                .await
                .unwrap();
        })
        .await;
...
#[tokio::test]
async fn oversized_incoming_frame_fails_connection() {
    let local_set = tokio::task::LocalSet::new();
    local_set
        .run_until(async {
            let agent = TestAgent::new();
            let (incoming_rx, mut incoming_tx) = piper::pipe(1024);
            let (_connection, io_task) = AgentSideConnection::new_with_limits(
                agent,
                futures::io::sink(),
                incoming_rx,
                ConnectionLimits::bounded(256, 128, 16),
                |future| {
                    tokio::task::spawn_local(future);
                },
            );
            ...
            incoming_tx.write_all(&[b'x'; 129]).await.unwrap();
            let result = tokio::time::timeout(Duration::from_secs(1), io_handle)
                .await
                .unwrap()
                .unwrap();
            assert!(result.is_err());
...
#[tokio::test]
async fn too_many_inflight_messages_fails_connection() {
    let local_set = tokio::task::LocalSet::new();
    local_set
        .run_until(async {
            let agent = TestAgent::new().with_blocked_prompts();
            let (incoming_rx, mut incoming_tx) = piper::pipe(64 * 1024);
            let (_connection, io_task) = AgentSideConnection::new_with_limits(
                agent,
                futures::io::sink(),
                incoming_rx,
                ConnectionLimits::bounded(1024, 1024, 256),
                |future| {
                    tokio::task::spawn_local(future);
                },
            );
            let io_handle = tokio::task::spawn_local(io_task);
            let input = "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"session/prompt\",\"params\":{\"sessionId\":\"test\",\"prompt\":[]}}\n"
                .repeat(257);

            incoming_tx.write_all(input.as_bytes()).await.unwrap();
            let result = tokio::time::timeout(Duration::from_secs(1), io_handle)
                .await
                .unwrap()
                .unwrap();
